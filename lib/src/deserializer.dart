...
class Deserializer {
  ...
  /// The [recurseLimit] parameter limits the nesting depth of arrays and maps
  /// to prevent stack overflow. Defaults to 500.
  Deserializer(
    Uint8List buffer, {
    ExtDecoder? extDecoder,
    int recurseLimit = 500,
  })  : _reader = BinaryReader(buffer),
        _extDecoder = extDecoder,
        _recurseLimit = recurseLimit;

  final BinaryReader _reader;
  final ExtDecoder? _extDecoder;
  final int _recurseLimit;
  int _depth = 0;
  ...
  ///
  /// Throws a [MessagePackError] if the buffer contains invalid MessagePack format.
  dynamic decode() {
    if (_depth >= _recurseLimit) {
      throw MessagePackError('Recursion limit exceeded');
    }
    _depth++;
    try {
